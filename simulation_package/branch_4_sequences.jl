module Branch4Sequences # Li_L2_L2_L2
# endogeneous parties: NO
# dynamic candidates: NO
...
function run_proportional_election_sequence(fixed_params::FixedParams{K},
    ...
    @inline function generate_parties(ideal_point_means::AbstractVector{Vector{Float64}},
        ...
        return party_ideal_points, engagement
    end
    ...
    @unpack n_parties, directional_utility, turnout_level, strategic_level = branch_params

    engagement, _ = PartySimulation.compute_engagement(
        voter_ideal_points, α_political_class, p_norm, n_issues, n_seats, pop_per_seat, rng
    )

    voter_ideal_points = PartySimulation.remodel_issue_space(voter_ideal_points, n_issues, n_seats,
        pop_per_seat, issue_dimensions)

    n_questions = [sum(n_questions)]
    n_positions = [n_positions[1]]
    issue_dimensions = [sum(issue_dimensions)]
    n_issues = 1

    voter_ideal_points = PartySimulation.recover_3d_after_pca(
        zscore(voter_ideal_points), issue_dimensions[1], n_seats, pop_per_seat
    )

    voter_ideal_points = [voter_ideal_points]

    ideal_means = [zeros(Float64, issue_dimensions[1])]
    ideal_variances = [ones(Float64, issue_dimensions[1])]

    party_ideal_points, engagement = generate_parties(ideal_means, ideal_variances, n_parties,
...
function run_majoritarian_sequence(fixed_params::FixedParams{K},
    ...
    function run_majoritarian_election(voter_ideal_points::AbstractVector{Array{Float64,3}},
        ...
        begin
            first_round_results, voter_utilities, first_round_candidate_choices =
                ElectionSimulation.run_single_round_election(voter_ideal_points, candidates,
    ...
    n_questions = [sum(n_questions)]
    issue_dimensions = [sum(issue_dimensions)] # BECAUSE NOT PCA'D
    n_issues = 1
